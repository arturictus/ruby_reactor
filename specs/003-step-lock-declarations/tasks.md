...
8. **The step contract is enforced inside `impl.run` (0.8.0).** `RubyReactor::Step.run` runs
   `enforce_contract!` (defaults + validation) before instantiating. Wrapping `impl.run` from
   outside would take the hold before validation and hand the key proc arguments without
   defaults (research Finding 7). So class-step forward acquisition lives INSIDE `Step.run`,
   after `enforce_contract!` (T013). The executor wraps only inline `run_block`s.
9. **`RubyReactor::Step` is a class with no hooks (0.8.0).** No `Step.included`, no
   `ClassMethods` module, and its header promises no `prepend`. Macros are hosted with a plain
   `extend` in the class body (T006); direct-invocation coordination is a plain call in
   `Step.run` (T037).
...
- Coordination source for a step: `step_config` for inline declarations, falling back to
  ...
  so rollback, the dispatch guard, and the dashboard only ever read `step_config.lock_config` and
  the other four. Forward acquisition for an inline step reads `StepConfig#inline_coordination?`
  (its own configs only), so a class step is never acquired by both the executor and `Step.run`.
- `Context#coordination_owner`: transient `attr_accessor`, NOT serialized. Overrides the owner
  for everything coordinated with that context (research D5).
...
- [X] T005 [P] Write `spec/ruby_reactor/step_coordination/declaration_spec.rb` and confirm it fails. Cover:
  - (a) A `RubyReactor::Step` subclass can call all five macros; the per-macro readers (`lock_config`, etc.) return the same hashes the reactor form builds.
...
- [X] T009 Create the `lib/ruby_reactor/executor/step_coordination.rb` skeleton with no enforcement yet.
  ...
  - `#owner`: the explicit `owner:`, else `context.coordination_owner`, else `(context.root_context || context).context_id`, else a memoized `SecureRandom.uuid` when the context is nil (full rule: T037).
...
- [X] T010 Run `declaration_spec.rb` and get it green. Run the full `bundle exec rspec`: no existing spec may change result (FR-027)
...
- [X] T011 [P] [US1] Write `spec/ruby_reactor/step_coordination/lock_spec.rb` against real Redis, using two `Thread`s running sync `Reactor.run`, each with `with_lock(wait: 5)` so the loser blocks instead of failing. Cover:
  ...
  - (6b) A step with `input :region, :string, optional: true, default: "eu"` and `with_lock { |a| "r:#{a[:region]}" }`, wired without `region`, locks `"r:eu"` (key sees the same `inputs` as the instance).
  - (6c) A step whose `input :amount, :integer` contract fails takes no lock and emits no `:lock_acquired` (Finding 8).
...
- [X] T012 [US1] Implement exclusive-lock acquisition in `StepCoordination#around_run`. When `step_config.lock_config` is present:
...
- [X] T013 [US1] Wire enforcement (Finding 8, research D2 table):
  - Class steps, `lib/ruby_reactor/step.rb`: in `Step.run`, after `validated = enforce_contract!(arguments)`, run `coordinate(validated, context) { catch(StepSignals::TAG) { new(validated, context).run } }`. Private `coordinate` yields straight through when `!declares_coordination?`, otherwise, with `ctx = (context if context.is_a?(RubyReactor::Context))`, `StepCoordination.new(step_config: self, arguments: validated, context: ctx, reactor_class: ctx&.reactor_class, middlewares: ctx&.middlewares || RubyReactor::MiddlewareRunner.new([])).around_run { yield }`. `Contended` and `KeyError` propagate to the caller. Update the lifecycle list in the `Step` header: coordination is step 2b, after contract enforcement, before instantiation.
  - Inline steps, `lib/ruby_reactor/executor/step_executor.rb#run_step_implementation`: in the `has_run_block?` branch only, after `inline_contract.enforce!`, wrap the `run_block` call in `StepCoordination.new(step_config:, arguments: args_to_pass, context: @context, reactor_class: @reactor_class, middlewares: @middlewares).around_run { ... }` when `step_config.inline_coordination?`.
  - Add `StepConfig#inline_coordination?` (true when any of its OWN five configs is set, ignoring the `impl` fallback).
  - The guard check and `validate_step_arguments` already run before `run_step_implementation`, so nothing is taken for a step that will not run (FR-012, D3).
  - `StepCoordination#step_name`: `context&.current_step || step_config.name.to_s` for a class, `step_config.name` for a `StepConfig`. Inside the executor `current_step` is the step's reactor name (set by `with_step`).
- [X] T014 [US1] In `StepExecutor#safe_execute_step_sync`, add `rescue StepCoordination::KeyError => e` BEFORE the generic `StandardError` rescue. Return `RubyReactor::Failure(e, retryable: false, step_name: step_config.name, reactor_name: @reactor_class.name, step_arguments: resolved_arguments, inputs: @context.inputs)`. Also add a temporary `rescue StepCoordination::Contended => e` returning the same shape with `exception_class: e.original.class.name`. US3 replaces this with the park/fail split
- [X] T015 [US1] Run `lock_spec.rb` until green. Fix `StepCoordination` or the wiring, not the spec
...
- [X] T018 [P] [US3] Write `spec/ruby_reactor/step_coordination/contention_spec.rb`, live-Sidekiq lane (`RealAsyncBackend.start_sidekiq!`, skip with reason if it cannot boot). Cover:
...
- [X] T019 [P] [US3] Add a contention counter to `lib/ruby_reactor/retry_context.rb`:
...
- [X] T022 [US3] Add public `RetryManager#park_for_contention(step_config, contended, reactor_class)`:
...
- [X] T023 [US3] Make sure `RetryManager#handle_retry_result` passes a `RetryQueuedResult` returned from the block through unchanged. It already does; add a comment saying contention parks rely on this
- [X] T024 [US3] Give back the failure-retry attempt on a contention park (Finding 2): add `RetryContext#decrement_attempt_for_step(name)` (floor 0) and call it at the start of `park_for_contention` before anything else, so `prepare_retry_attempt`'s increment for this round is undone
- [X] T025 [US3] Replace T014's temporary `Contended` rescue in `StepExecutor#safe_execute_step_sync` with the park/fail split:
...
- [X] T026 [US3] Keep reactor-level holds across a contention park (FR-018, US3-3). In `Executor#resume_execution` (`lib/ruby_reactor/executor.rb`), after `@result` is computed: if `@result.is_a?(RetryQueuedResult) && @context.private_data[:step_contention]`, call `park_held_primitives!`, which sets `@parked` so the ensure skips `release_locks`. Then confirm that `consume_parked_primitives!` / reattach runs on the redelivery path that `requeue_job` → `Worker#perform` → `resume_execution` takes. If reattach is only wired for the `AsyncResultPending` redelivery, generalize the check to any resume that finds `private_data[:parked_primitives]`
- [X] T027 [US3] Verify the map path in `lib/ruby_reactor/map/element_executor.rb`: a `RetryQueuedResult` from a contention park must take the existing "async retry requeued this element" branch. Adjust the condition only if it keys on something other than the result class
- [X] T028 [US3] Run `contention_spec.rb` until green, then re-run `lock_spec.rb` and `scope_spec.rb`
...
- [X] T029 [P] [US4] Write `spec/ruby_reactor/step_coordination/reentrancy_spec.rb` and cover:
  ...
  - (9) An executor-driven class step records exactly one `:lock_acquired` for K (acquired once, in `Step.run`, never also by the executor).
  - (10) Stand-alone vs a running reactor: a reactor holds K (reactor-level `with_lock`) and sleeps in a step; from another thread `LockedChargeStep.run({ account_id: 1 }, nil)` with `wait: 0` fails with contention. Same with the reactor's hold being a STEP-level hold on K (US4-7).
  - (11) Two stand-alone calls on K from two threads never overlap (`max_concurrency == 1`), and with different keys they overlap (US4-7).
  - (12) A locked step's body calls `LockedChargeStep.run({ account_id: 1 }, context)` (same key, same execution): proceeds with `wait: 0`, and K stays locked by the root id after the inner call returns (US4-8, FR-020).
  - (13) The same body calls `LockedChargeStep.run({ account_id: 2 }, context)` while another execution holds `acct:2`: the inner call contends (fails with `wait: 0`) — being inside a locked step exempts nothing but its own key (US4-8).
  - (14) The same body calls `LockedChargeStep.run({ account_id: 1 }, nil)` (no context): contends with its own execution's hold and fails with `wait: 0`. Documents that re-entrancy requires passing the execution.
  - (15) Composed child reactor with a step locking K inside a parent step locking K: completes with `wait: 0` (reactor → step → compose → step).
  - (16) `Context#coordination_owner` is not present in `ContextSerializer.serialize` output.

### Implementation for User Story 4

- [X] T030 [US4] Add `attr_accessor :coordination_owner` to `lib/ruby_reactor/context.rb` (transient; confirm `ContextSerializer` never writes it). Add the `# re-entrancy: same owner as every reactor in this execution tree` comment on `StepCoordination#owner`
- [X] T031 [US4] Fix `Executor#release_locks` in `lib/ruby_reactor/executor.rb` (Finding 1). Replace both `held_lock_keys.delete(key)` calls with a single-occurrence pop (`i = held_lock_keys.index(key); held_lock_keys.delete_at(i) if i`). Extract it as private `pop_held_lock_key(key)` so `StepCoordination#pop_key` has the same semantics
- [X] T032 [US4] Also push the step's semaphore key to the registry when `limit == 1`, in `StepCoordination`, matching `Executor#acquire_semaphore`. The semaphore itself is implemented in US5 (T043); this task is the registry rule only, written so T043 calls it
- [X] T033 [US4] Make the deadlock guard reusable from `lib/ruby_reactor/step/async_reactor_step.rb`:
...
- [X] T034 [US4] Add a dispatch-time guard in `lib/ruby_reactor/executor/async_step_dispatch.rb#dispatch_async_step`, before `record_async_step_dispatch`:
...
- [X] T035 [US4] Async_step worker, `lib/ruby_reactor/step_worker.rb`. Class steps are already coordinated by `Step.run` (T013); the worker only fixes the owner and wraps inline bodies:
  - In `load_step_context`, set `found.coordination_owner = SecureRandom.uuid` (per job, NEVER the root id: US4-5, D5 "ownership never crosses a hand-off"). Add the `coordination_owner` accessor to `lib/ruby_reactor/context.rb` and confirm `ContextSerializer` does not persist it.
  - In `execute_step_body`'s `has_run_block?` branch, wrap the call as T013 does for inline steps.
  - The context is marked `inline_async_execution`, so `wait_for` returns 0. Rescue `StepCoordination::KeyError` into a Failure record through `complete`.
- [X] T036 [US4] Park the async_step body on contention (Finding 6):
...
- [X] T037 [US4] Owner resolution in `StepCoordination#owner` (research D5), covering direct invocation (FR-023) with no extra code path since T013 already coordinates inside `Step.run`:
  - `owner:` kwarg if given, else `context&.coordination_owner`, else `(context.root_context || context).context_id` for a Context, else a per-call `SecureRandom.uuid` memoized on the `StepCoordination` instance.
  - Direct calls never park (`park` is true only when `context&.inline_async_execution`, and then the caller that can park handles `Contended`).
  - `.undo` / `.compensate` do not coordinate in the base class: rollback coordination is the executor's (T046/T047). Say so on both methods.
  - No thread-local "already coordinated" mark. Re-entrancy comes only from the owner; a nested call with the same owner and key increments the adapter's count, a different key is taken normally.
- [X] T038 [US4] Run `reentrancy_spec.rb` until green. Re-run `spec/ruby_reactor/dsl/async_reactor_locks_spec.rb` and `spec/ruby_reactor/integration/locking_spec.rb` unchanged (SC-012)
...
- [X] T039 [P] [US5] Write `spec/ruby_reactor/step_coordination/primitives_spec.rb` (semaphore, rate limit, period, ordering) and cover:
...
- [X] T040 [US5] Restructure `StepCoordination#around_run` into the fixed order (contract §3, D3) so each primitive is a small private method:
...
- [X] T041 [US5] Implement the rate limit in `StepCoordination`, mirroring `Executor#check_rate_limit`:
...
- [X] T044 [US5] Run `primitives_spec.rb` (non-ordered sections) until green
...
- [X] T045 [P] [US6] Write `spec/ruby_reactor/step_coordination/rollback_spec.rb` and cover:
...
- [X] T046 [US6] Implement `StepCoordination#around_rollback { }`:
...
- [X] T049 [P] [US7] Write `spec/ruby_reactor/step_coordination/observability_spec.rb` and cover:
...
- [X] T050 [US7] Emit `:failed_step` correctly around parks in `StepExecutor#execute_step` (`lib/ruby_reactor/executor/step_executor.rb`). A `RetryQueuedResult` from a contention park must go to `:complete_step`, as it does today for retry requeues; confirm this and add a comment. Add the structured `parked` log line in the T025 park branch, using the existing `log_async_event` formatting (extend it to accept extra key=value fields)
- [X] T051 [US7] Attribute step events in `lib/ruby_reactor/open_telemetry.rb`: where lock/semaphore events are handled, add a `ruby_reactor.step` span attribute from `context.current_step` when present. If those events are not handled there, skip it and note that in the task
- [X] T052 [US7] Extend `lib/ruby_reactor/web/coordination_serializer.rb` with `build_steps(reactor_class, context)`:
...
- [X] T053 [US7] Render step-level entries in the dashboard coordination panel under `lib/ruby_reactor/web/public/`: one row per step with step/primitive/key/state/owner, plus a "waiting on" badge for `waiting`, visually distinct from failure
- [X] T054 [US7] Add a matcher `have_contended_at(step_name)` to `lib/ruby_reactor/rspec/matchers.rb`. It matches a result/context whose `execution_trace` contains `type: :contention_park, step: step_name`, with an optional `.on(key)` chain. The demo spec needs it (Constitution VI), and it doubles as the shipped way to assert US7-4
- [X] T055 [US7] Run `observability_spec.rb` until green
...
- [X] T056 [P] [US8] Write `spec/ruby_reactor/step_coordination/inline_spec.rb`:
...
- [X] T057 [US8] Run `inline_spec.rb`. The inline path is expected to work already via T007 + T013 (`run_block` goes through `run_step_implementation`). Fix only real gaps, such as `StepCoordination` reading `step_config.name` for attribution when there is no `impl`
...
- [X] T058 [P] [US5] Add an ordered section to `spec/ruby_reactor/step_coordination/primitives_spec.rb`, live-Sidekiq lane. Cover:
...
- [X] T059 [US5] Document the weaker guarantee on the macro itself in `lib/ruby_reactor/dsl/lockable.rb#with_ordered_lock`'s doc comment: "On a step, the position is assigned when the execution first REACHES the step (its key reads step arguments), so executions are ordered by arrival at that step, not by enqueue. Identical to the reactor form only when the step is first in its reactor." (research D8, contract §1)
- [X] T060 [US5] Implement the ordered gate as the first stage of `StepCoordination#around_run`:
...
- [X] T061 [US5] Add a per-step heartbeat and advance-on-terminal in `StepCoordination`:
...
- [X] T062 [US5] Exclude the ordered lock from `around_rollback` (T046 already takes only lock/semaphore; add a spec-referenced comment), and exclude it from the async_step dispatch guard (T034 only considers lock and limit-1 semaphore)
- [X] T063 [US5] Run the ordered section of `primitives_spec.rb` until green
...
- [X] T064 [P] Update `documentation/locks_and_semaphores.md` with a new "Step-scoped coordination" section:
  ...
  - Extend the existing `### Owner identity` and `### Re-entrancy` sections with the step owner rule (research D5): same execution = root id (reactor → step → compose → `Step.run(args, context)` from a body); a stand-alone `Step.run(args)` is its own execution and contends with running reactors; an `async_step` worker uses a per-job owner; re-entrancy never exempts a different key. Include the "call without `context` from inside its own locked body waits on itself" caveat.
  - Extend `### Inline vs async behavior on contention` and `## Snooze configuration`: step contention parks the execution at that step, reusing `lock_snooze_*`, and does not consume the step's `retries` budget.
  - Extend `## Observability` (events attributed via `context.current_step`, `private_data[:step_contention]`, the `contention_park` trace entry, dashboard step rows) and `## Inheritance` (step subclasses inherit declarations; redeclaring replaces).
  - Update `## Limitations`: no coordination on interrupt steps; `.undo`/`.compensate` called directly take no hold.
  - The rollback table.
...
- [X] T065 [P] Update `README.md`: add a short step-level `with_lock` example to the locking section, linking to `documentation/locks_and_semaphores.md`
- [X] T075 [P] Update `documentation/async_reactors.md`:
  - The `async_step` bullet "The step's work runs **outside** the reactor's `lock` / `semaphore` / `rate_limit` windows … must arrange it itself" is now wrong for step-declared coordination. Rewrite: the reactor's own windows still do not cover the body, but coordination the step declares is taken **in the worker**, with a per-job owner (never the dispatcher's), and a contended body parks via `perform_step_in`.
  - The dispatch-time deadlock section ("if the child declares an exclusive `lock` …") now also covers `async_step` whose step class declares a held key; add the `async_step` remedy (drop `async_step` if the work belongs inside the critical section) and the non-blocking-resolution caveat (guard skipped, logged as `step_coordination.guard_skipped`, when args depend on a pending async result).
- [X] T076 [P] Update `documentation/core_concepts.md` `### Step Classes (preferred)`: show a class step with `with_lock`, state the lifecycle order (contract → coordination → instance → signals), and that direct `MyStep.run(args)` is coordinated too. In `## Retries`, one line: contention parks are not retries and do not consume `max_attempts`.
- [X] T077 [P] Update `documentation/middlewares.md` `### Async, Locks & Semaphores`: lock/semaphore/rate-limit events now also fire for step-level holds, with `context.current_step` naming the step (nil for reactor-level); add the parked-contention log line.
- [X] T078 [P] Update `documentation/testing.md` `## Custom RSpec Matchers`: document `have_contended_at(step).on(key)` (T054) and, if added, the `hold_lock(key, owner:)` helper (T069), with a step-lock example under `## Testing Step Classes` including a direct `MyStep.run` contention assertion.
- [X] T079 [P] Update `documentation/interrupts.md` and `documentation/retry_configuration.md` with one short note each: interrupts refuse coordination macros (point to reactor-level); step contention is separate from retry attempts and bounded by `lock_snooze_max_attempts`.
- [X] T080 [P] Update the indexes `documentation/README.md` (and `README.md`'s docs list if present) so "step-scoped coordination" is findable, linking `locks_and_semaphores.md#step-scoped-coordination`.
- [X] T066 [P] Add a `CHANGELOG.md` entry under `Features`: "Steps can declare `with_lock`, `with_semaphore`, `with_rate_limit`, `with_period`, `with_ordered_lock`, keyed on their own arguments" (MINOR, additive)
- [X] T067 [P] Create `demo_app/app/reactors/step_lock_demo_reactor.rb` (class `StepLockDemoReactor`) with class-based steps only:
...
- [X] T068 Register `demo:step_lock` in `demo_app/lib/tasks/demo_reactors.rake` with a `desc` and `[:environment, :flush_redis]`. It prints three sections:
...
- [X] T069 Write `demo_app/spec/reactors/step_lock_demo_reactor_spec.rb` (`type: :reactor`) using ONLY `lib/ruby_reactor/rspec.rb` surface:
...
- [X] T070 Check `docker-compose.yml`: `demo-redis` and `sidekiq` must be present and wired so `docker compose run --rm demo-app bin/rails demo:step_lock` needs no manual setup (Constitution VI.4). Add a missing service or env var in the same change
- [X] T071 Run `docker compose run --rm demo-app bundle exec rspec spec/reactors/step_lock_demo_reactor_spec.rb` and `docker compose run --rm demo-app bin/rails demo:step_lock`. Confirm the three printed sections match quickstart Scenario 6 (SC-013)
- [X] T072 Run full `bundle exec rspec` and `bundle exec rubocop` (no `--disable-pending-cops`). Compare against `specs/003-step-lock-declarations/baseline.txt` (T001): no previously passing example may fail (SC-012). Then delete `baseline.txt`
- [X] T081 Grep `documentation/` and `README.md` for statements this feature falsifies (`outside\*\* the reactor's`, `must arrange it itself`, `with_lock` described as reactor-only, `@yield [inputs]` wording) and fix any T064–T080 missed. Also update `Dsl::Lockable` YARD comments: `@yield [inputs]` → reactor inputs or step `inputs`; "for this reactor" → "for this reactor or step"
- [X] T073 Walk the quickstart.md acceptance checklist (SC-001…SC-013), map each row to the spec example that proves it, and fix any gap
- [X] T074 Update the `Complexity Tracking` table in `specs/003-step-lock-declarations/plan.md` if the ordered-lock phase was cut or deferred. Mark data-model.md's `ContentionState` as superseded by `RetryContext#contention_attempts` + `lock_snooze_*` config (Finding 7)
...
- **Polish (Phase 12)**: T067–T071 need US1, US3, US6, and T054. Docs (T064–T066, T075–T080) can start after US4; T078 needs T054
...
- T064, T065, T066, T075–T080, T067 (docs and demo reactor) in parallel
...
- The line numbers cited were taken on `step_validations` (`845b010`) and re-checked on
  `independent_step_locks` after 0.8.0 (`10f0911`). Re-locate by method name if they drift
