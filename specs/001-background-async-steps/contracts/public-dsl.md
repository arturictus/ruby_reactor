...
- **Runtime contract**:
  ...
  - On a valid payload: result stored, context persisted (job payload is identity-only), `before_async_enqueue` middleware fires, remainder enqueued via `configuration.async_router`, and `continue` returns an `AsyncResult` (with `execution_id`) instead of the final result.
